...
if (player?.emailAddress) {
  await sendMail(
    ...
    `<p>Coach <strong>${req.user.firstName || req.user.username}</strong> submitted feedback for your session on <em>${new Date(session.date).toLocaleDateString()}. Click on https://cricket-academy-frontend-px1s.onrender.com/login to login</em>.</p>`
  );
}
