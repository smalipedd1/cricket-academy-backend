...
router.patch('/player/:id', verifyRole('coach'), async (req, res) => {
  try {
    ...
    // ✅ Only hash if a new password is explicitly provided
    if (updates.password && updates.password.trim() !== '') {
      updates.password = await bcrypt.hash(updates.password, 10);
    } else {
      delete updates.password; // prevent accidental rehash
    }
...
//addition for email notification
const player = await Player.findById(entry.playerId);
if (player?.emailAddress) {
  await sendMail(
    ...
    `<p>Coach <strong>${req.user.firstName || req.user.username}</strong> submitted feedback for your session on <em>${new Date(session.date).toLocaleDateString()}</em>.<br/>Login to view: <a href="https://cricket-academy-frontend-px1s.onrender.com">Academy Portal</a></p>`
  );
}
...
// ✅ GET full coach profile (used in evaluation form)
router.get('/profile', verifyRole('coach'), async (req, res) => {
  try {
    const coach = await Coach.findById(req.user._id).select('_id firstName lastName email role');
    if (!coach) return res.status(404).json({ error: 'Coach not found' });

    res.json(coach); // ✅ returns full object with _id
  } catch (err) {
