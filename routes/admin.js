...
router.post('/sessions', verifyRole('admin'), async (req, res) => {
  const {
    ...
    isRecurring,
    recurrencePattern,
    recurrenceWeeks = 1, // default to 1 if not provided
  } = req.body;
  ...
  try {
    ...
    const { dayOfWeek, recurrenceGroupId } = recurrencePattern;
    ...
    for (let i = 0; i < recurrenceWeeks; i++) {
      ...
      const session = new Session({
        ...
        performance,
        isRecurring: true,
        recurrencePattern: { dayOfWeek, recurrenceGroupId },
      });
...
// Delete a session
router.delete('/sessions/:id', verifyRole('admin'), async (req, res) => {
  try {
